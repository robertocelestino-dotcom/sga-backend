...
public class FranquiaRuleOLD {
	 ...
	 * 
	 * Regras: 1. Identificar item da franquia (contém "FRANQUIA DE CONSULTA") 2.
	 * Extrair a quantidade da franquia do próprio item - este é o LIMITE 3. Extrair
	 * o nome do serviço vinculado (removendo o prefixo "FRANQUIA DE CONSULTA") 4.
	 * Buscar o item de serviço correspondente na fatura (EXCLUINDO o próprio item
	 * da franquia) 5. SEMPRE remover o item da franquia 6. Se consumo > franquia:
	 * atualizar serviço com excedente 7. Se consumo <= franquia: remover o serviço
	 * também
	 */
	public Fatura aplicarRegraFranquia(Fatura fatura, Associado associado) {
		...
		Optional<FaturaItem> franquiaItemOpt = fatura.getItens().stream().filter(
				item -> item.getDescricao() != null && item.getDescricao().toUpperCase().contains(PREFIXO_FRANQUIA))
				.findFirst();
		...
		BigDecimal quantidadeFranquiaBD = franquiaItem.getQuantidade() != null ? franquiaItem.getQuantidade()
				: BigDecimal.ZERO;
		...
		BigDecimal quantidadeServicoBD = servicoItem.getQuantidade() != null ? servicoItem.getQuantidade()
				: BigDecimal.ZERO;
		...
		if (quantidadeServico > limiteFranquia) {
			...
			servicoItem.setValorTotal(servicoItem.getQuantidade().multiply(servicoItem.getValorUnitario()));
			log.info("✅ Franquia EXCEDIDA!");
			log.info("   Limite: {}, Consumo: {}, Excedente: {}", limiteFranquia, quantidadeServico, excedente);
			log.info("   Serviço atualizado: Quantidade={}, Valor=R$ {}", excedente, servicoItem.getValorTotal());
		} else {
	...
	/**
	 * Extrai o nome do serviço a partir da descrição da franquia Remove o prefixo
	 * "FRANQUIA DE CONSULTA" e retorna o restante Ex: "FRANQUIA DE CONSULTA SPC MIX
	 * POSITIVO FOR" -> "SPC MIX POSITIVO FOR"
	 */
	private String extrairNomeServicoDaFranquia(String descricaoFranquia) {
		if (descricaoFranquia == null)
			return "";

		String nome = descricaoFranquia.toUpperCase().replace(PREFIXO_FRANQUIA, "").trim();
