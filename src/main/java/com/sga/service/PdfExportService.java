...
public class PdfExportService {
    ...
    public byte[] gerarPdfResumoImportacao(Long importacaoId, Map<String, Object> resumo) {
        ...
        try {
            ...
            context.setVariable("dataGeracao",
    ...
    private String gerarHtmlFatura(Fatura fatura, List<FaturaItem> itens) {
        ...
        html.append(
                ".total-geral { text-align: right; font-size: 18px; font-weight: bold; margin-top: 20px; padding-top: 10px; border-top: 2px solid #333; }");
        html.append(
        ...
        html.append("        <td class='label'>Status:</td><td><span class='status status-").append(fatura.getStatus())
                .append("'>").append(fatura.getStatus()).append("</span></td></tr>");
        html.append("    <tr><td class='label'>Data Emissão:</td><td>")
                .append(formatarDataLocal(fatura.getDataEmissao())).append("</td>");
        html.append("        <td class='label'>Data Vencimento:</td><td>")
                .append(formatarDataLocal(fatura.getDataVencimento())).append("</td></tr>");
        html.append("    <tr><td class='label'>Associado:</td><td colspan='3'>")
                .append(fatura.getAssociado() != null ? fatura.getAssociado().getNomeRazao() : "N/A")
                .append("</td></tr>");
        html.append("    <tr><td class='label'>CNPJ/CPF:</td><td colspan='3'>")
                .append(fatura.getAssociado() != null ? fatura.getAssociado().getCnpjCpf() : "N/A")
                .append("</td></tr>");
        if (fatura.getAssociado() != null && fatura.getAssociado().getCodigoSpc() != null) {
            html.append("    <tr><td class='label'>Código SPC:</td><td colspan='3'>")
                    .append(fatura.getAssociado().getCodigoSpc()).append("</td></tr>");
        }
        if (fatura.getNumeroRps() != null) {
            html.append("    <tr><td class='label'>Nº RPS:</td><td colspan='3'>").append(fatura.getNumeroRps())
                    .append("</td></tr>");
        }
        if (fatura.getObservacao() != null && !fatura.getObservacao().isEmpty()) {
            html.append("    <tr><td class='label'>Observação:</td><td colspan='3'>").append(fatura.getObservacao())
                    .append("</td></tr>");
        }
        html.append("  </table>");
        html.append("</div>");
        ...
        html.append("  <thead>");
        html.append(
                "    <tr><th>Código</th><th>Descrição</th><th style='text-align:right'>Qtd</th><th style='text-align:right'>Vl. Unit.</th><th style='text-align:right'>Vl. Total</th></tr>");
        html.append("  </thead>");
        html.append("  <tbody>");
        ...
        if (itens != null && !itens.isEmpty()) {
            for (FaturaItem item : itens) {
                html.append("    <tr>");
                html.append("      <td>").append(item.getCodigoProduto() != null ? item.getCodigoProduto() : "-")
                        .append("</td>");
                html.append("      <td>").append(item.getDescricao() != null ? item.getDescricao() : "-")
                        .append("</td>");
                html.append("      <td style='text-align:right'>")
                        .append(item.getQuantidade() != null ? item.getQuantidade() : 0).append("</td>");
                html.append("      <td style='text-align:right'>").append(formatarMoeda(item.getValorUnitario()))
                        .append("</td>");
                html.append("      <td style='text-align:right'>").append(formatarMoeda(item.getValorTotal()))
                        .append("</td>");
                html.append("    </tr>");
            }
        } else {
            html.append(
                    "    <tr><td colspan='5' style='text-align:center; color:#999;'>Nenhum item encontrado</td></tr>");
        }
    ...
    // ============================================================
    // 🔥 MÉTODO GERAR NOTA DE DÉBITO PDF (CORRIGIDO)
    // ============================================================
    
    /**
     * Gera PDF da nota de débito usando o método existente gerarPdfNota()
     * 
     * @param nota NotaDebitoSPC para gerar o PDF
     * @return byte[] do PDF gerado
     */
    public byte[] gerarNotaDebitoPdf(NotaDebitoSPC nota) {
        try {
            // 🔥 CORRIGIDO: Usar o método existente gerarPdfNota(nota.getId())
            return gerarPdfNota(nota.getId());
        } catch (Exception e) {
            logger.error("❌ Erro ao gerar PDF da nota {}: {}", nota.getId(), e.getMessage());
            throw new RuntimeException("Erro ao gerar PDF da nota: " + e.getMessage());
        }
    }
}