...
public class FaturamentoExtemporaneoService {
    ...
    /**
     * 🔥 PROCESSAR FATURAMENTO EXTEMPORÂNEO
     * Réguas ID 7 e 8
     * 
     * Fórmula: QtdeFaturada = (Consolidado - Prévia do Dia/Mês Anterior) + Prévia do Dia/Mês Corrente
     */
    public List<FaturaItem> processarExtemporaneo(List<NotaDebitoSPC> notas) {
        ...
        for (Map.Entry<String, ItemSPC> entry : consolidadoMap.entrySet()) {
            ...
            BigDecimal qtdeCalculada = qtdeConsolidada
                .subtract(qtdePreviaAnterior)
                .add(qtdePreviaCorrente);

            log.debug("   🔢 {}: Consolidado={}, PréviaAnterior={}, PréviaCorrente={}, Calculada={}", 
            ...
            if (qtdeCalculada.compareTo(BigDecimal.ZERO) > 0) {
                ...
                log.info("   ✅ ADICIONADO: {} - Qtd: {}, Valor: R$ {}", 
                    item.getDescricao(), item.getQuantidade(), item.getValorTotal());
            } else {
        ...
        for (Map.Entry<String, ItemSPC> entry : previaCorrenteMap.entrySet()) {
            ...
            log.debug("   🔢 {} (apenas corrente): PréviaAnterior={}, PréviaCorrente={}, Calculada={}", 
                chave, qtdePreviaAnterior, qtdePreviaCorrente, qtdeCalculada);
            
            if (qtdeCalculada.compareTo(BigDecimal.ZERO) > 0) {
                FaturaItem item = criarFaturaItem(itemPreviaCorrente, qtdeCalculada);
                itensFatura.add(item);
                
                log.info("   ✅ ADICIONADO (corrente): {} - Qtd: {}, Valor: R$ {}", 
                    item.getDescricao(), item.getQuantidade(), item.getValorTotal());
            } else {
                log.info("   ⏭️ IGNORADO (Qtd <= 0): {} - Qtde: {}", chave, qtdeCalculada);
            }
     ...
     * 
     * Regra: Ordena por data_fim_periodo e classifica como:
    ...
    private Map<String, List<ItemSPC>> classificarNotasPorTipo(List<NotaDebitoSPC> notas) {
        ...
        List<NotaDebitoSPC> notasOrdenadas = notas.stream()
            .filter(n -> n.getDataFimPeriodo() != null)
            .sorted(Comparator.comparing(NotaDebitoSPC::getDataFimPeriodo))
    ...
    private FaturaItem criarFaturaItem(ItemSPC item, BigDecimal quantidade) {
        ...
        log.debug("   🔧 Item criado: {} | Código: {} | Qtd: {} | Valor: {}", 
            faturaItem.getDescricao(), faturaItem.getCodigoProduto(), 
    ...
    private String buscarCodigoProdutoRM(String descricaoServico) {
        ...
        try {
            ...
            // Busca exata (ignorando acentos e maiúsculas)
            for (Produto produto : todosProdutos) {
                if (produto.getDescricao() != null) {
                    String descricaoProdutoSemAcento = removerAcentos(produto.getDescricao().trim());
                    if (descricaoProdutoSemAcento.equalsIgnoreCase(descricaoSemAcento)) {
                        String codigoRM = produto.getCodigoRm();
                        if (codigoRM != null && !codigoRM.trim().isEmpty()) {
                            log.debug("   ✅ Mapeado: '{}' -> '{}'", descricaoServico, codigoRM);
                            return codigoRM;
                        }
                    }
                }
            ...
            for (Produto produto : todosProdutos) {
                if (produto.getDescricao() != null) {
                    String descricaoProdutoSemAcento = removerAcentos(produto.getDescricao().trim());
                    if (descricaoProdutoSemAcento.contains(descricaoSemAcento) || 
                        descricaoSemAcento.contains(descricaoProdutoSemAcento)) {
                        String codigoRM = produto.getCodigoRm();
                        if (codigoRM != null && !codigoRM.trim().isEmpty()) {
                            log.debug("   ✅ Mapeado (parcial): '{}' -> '{}'", descricaoServico, codigoRM);
                            return codigoRM;
                        }
                    }
                }
            ...
            log.warn("   ⚠️ Produto não encontrado para: '{}'", descricaoServico);
            return null;
        } catch (Exception e) {
            log.error("   ❌ Erro ao buscar produto para '{}': {}", descricaoServico, e.getMessage());
            return null;
        }
    }
